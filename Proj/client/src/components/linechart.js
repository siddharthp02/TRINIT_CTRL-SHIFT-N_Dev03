...
class LineChart extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    const timeDomain = d3.extent(data, d =>  d[Object.keys(d)[0]]);
    const tempMax = d3.max(data, d =>  d[Object.keys(d)[1]]);
    xScale.domain(timeDomain);
    yScale.domain([0, tempMax]);

    // calculate line for lows
    lineGenerator.x(d => xScale( d[Object.keys(d)[0]]));
    lineGenerator.y(d => yScale( d[Object.keys(d)[1]]));
